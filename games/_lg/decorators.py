...
def check_valid_vote(func):
    ...
    async def wrapper(self, ctx: discord.ApplicationContext, member: discord.Member, *args, **kwargs):
        ...
        if Roles.LG_VIVANT.value not in [role.id for role in member.roles] and Roles.LG_MORT.value in [role.id for role
                                                                                                       in member.roles]:
        ...
        if (Roles.LG_VIVANT.value not in [role.id for role in member.roles] and Roles.LG_MORT.value not in
